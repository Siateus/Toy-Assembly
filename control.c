...
void controle(char matriz[100][17], int n) {
    ...
    for (int i = 0; i < n; ) {
        ...
        if(strncmp(matriz[i], "ADD", 3) == 0 || strncmp(matriz[i], "SUB", 3) == 0 ||
        ...
        if(strncmp(matriz[i], "BEQ", 3) == 0 || strncmp(matriz[i], "BLT", 3) == 0 || strncmp(matriz[i], "JMP", 3) == 0) {
            if(strncmp(matriz[i], "JMP", 3) == 0) {
                ...
                i = (resposta == 0) ? indice : i; // Comentário: Pula para o índice se a resposta for falsa (0).
            }
        }
        ...
        if(strncmp(matriz[i], "EXIT", 4) == 0 || count == 100000){
